...
def lambda_handler(event, context):
    ...
    time_stamp = str(call_details.get("Participants", [{}])[0].get("StartTimeInMilliseconds", 0))
    ...
    if handler_fn:
        return handler_fn(event, transaction_id, call_id, call_details,time_stamp)
...
# ---------------------------------------------------------------------------
# NEW_INBOUND_CALL – answer and call and brdige (hand off to audio connector)
# ---------------------------------------------------------------------------

def handle_new_call(event, transaction_id, call_id, call_details, time_stamp    ):
    ...
    dynamo.put_item(
        ...
        Item={
            ...
            "Attributes": {"S": json.dumps(event) },
            "status":        {"S": "ACTIVE"},
            "TTL": {"N": str(math.floor(datetime.now(timezone.utc).timestamp()) + 300)},
            "CallId":        {"S": call_id},
...
def handle_action_successful(event, transaction_id, call_id, call_details, time_stamp):
...
def handle_action_failed(event, transaction_id, call_id, call_details, time_stamp):
...
def handle_call_update_requested(event, transaction_id, call_id, call_details, time_stamp):
...
def handle_hangup(event, transaction_id, call_id, call_details, time_stamp):
    caller_number = call_details.get("Participants", [{}])[0].get("From", "unknown")
    _cleanup_session(caller_number ,transaction_id,time_stamp)
    return {"SchemaVersion": "1.0", "Actions": []}
...
def _cleanup_session(caller_number,transaction_id,time_stamp):
    try:
        dynamo.delete_item(
            TableName=SESSIONS_TABLE,
            Key = {"CallerNumber":  {"S": caller_number} , 
                   "Timestamp":     {"N": time_stamp} }
        ...
        logger.error("Failed to clean up session %s - %s: %s", caller_number , transaction_id, exc)
