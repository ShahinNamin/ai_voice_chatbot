...
ROLE_NAME="AmazonBedrockAgentCoreSDKRuntime-${AWS_REGION}-webrtc-kvs"
...
cat > /tmp/runtime-policy.json << EOF
{
    ...
    "Statement": [
        ...
        },
        {
            ...
            "Resource": [
                "arn:aws:kinesisvideo:${AWS_REGION}:${ACCOUNT_ID}:channel/voice-agent-turn/*"
            ]
        }
    ]
}
...
echo "  ✅ KVS signaling channel access (for managed TURN credentials)"
echo ""
