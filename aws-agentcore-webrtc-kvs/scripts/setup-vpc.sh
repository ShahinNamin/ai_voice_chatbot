...
VPC_ID=$(aws ec2 create-vpc \
    ...
    --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=agentcore-webrtc-kvs-vpc}]' \
    --region $AWS_REGION \
    --query 'Vpc.VpcId' \
    --output text)
...
IGW_ID=$(aws ec2 create-internet-gateway \
    --tag-specifications 'ResourceType=internet-gateway,Tags=[{Key=Name,Value=agentcore-webrtc-kvs-igw}]' \
    --region $AWS_REGION \
...
SG_ID=$(aws ec2 create-security-group \
    --group-name agentcore-webrtc-kvs-sg \
    --description "Security group for AgentCore WebRTC runtime" \
    --vpc-id $VPC_ID \
    --tag-specifications 'ResourceType=security-group,Tags=[{Key=Name,Value=agentcore-webrtc-kvs-sg}]' \
    --region $AWS_REGION \
    --query 'GroupId' \
    --output text)
