...
# ElevenLabs STT expects 16 kHz PCM16 input.
# ElevenLabs TTS outputs 16 kHz PCM16 by default.
# This is fixed — not configurable — because ElevenLabs defines the rate.
    ...
    async def run(self, track):  # track: aiortc MediaStreamTrack (RemoteStreamTrack)
        ...
        try:
            while not self._stop.is_set():
                ...
                sr = frame.sample_rate or AGENT_SAMPLE_RATE
                ...
                if session.webrtc_frames_rx <= 3:
                    ...
                    if fmt in ('fltp', 'flt') and len(plane0_bytes) >= 16:
                        ...
                        log.info("WebRTC frame #%d  fmt=%s  layout=%s  sr=%d  samples=%d  "
                                 "planes=%d  plane0_bytes=%d  first4_floats=%s  call-id=%.36s",
                                 session.webrtc_frames_rx, fmt, frame.layout.name,
                                 sr, frame.samples, len(frame.planes),
                                 len(plane0_bytes), [f"{v:.4f}" for v in first4],
                                 session.call_id)
                    else:
                        log.info("WebRTC frame #%d  fmt=%s  layout=%s  sr=%d  samples=%d  "
                                 "planes=%d  plane0_bytes=%d  call-id=%.36s",
                                 session.webrtc_frames_rx, fmt, frame.layout.name,
                                 sr, frame.samples, len(frame.planes),
                                 len(plane0_bytes), session.call_id)
                if fmt in ('fltp', 'flt', 's16p', 's16', 's32p', 's32',
                    ...
                    if fmt in ('fltp', 'flt'):
                        ...
                        # int16 — take plane 0 (correct for s16p mono).
                        # BUG FIX: planes are alignment-padded; slice to
                        # exactly n_samples * 2 bytes to avoid garbage samples.
                        raw = bytes(frame.planes[0])[:n_samples * 2]
                        if fmt == 's16':
                            # interleaved: step by n_planes to extract channel 0
                            if n_planes == 1:
                                pcm16 = raw
                            else:
                                shorts = _struct.unpack_from(f'<{len(raw)//2}h', raw)
                                pcm16 = _struct.pack(
                                    f'<{n_samples}h',
                                    *[shorts[i * n_planes] for i in range(n_samples)]
                                )
                        else:
                            pcm16 = raw  # s16p plane 0 = mono channel
                    else:
        ...
        try:
            while not stop.is_set() and session.state != CallState.TEARING_DOWN:
                ...
                if session.rtp_rx_count == 1:
                    log.info("RTP->WebRTC: first RTP  ptype=%d  call-id=%.36s", ptype, session.call_id)
                ...
                if ptype == RTP_PAYLOAD_PCMU:
                    ...
                    continue  # skip non-audio (DTMF etc.)
...
async def main():
    ...
    log.info("  Audio rates       : Chime=%dHz (G.711 20ms)  Agent=%dHz (ElevenLabs PCM16)",
